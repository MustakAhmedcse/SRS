...
| Date | Decision |
|---|---|
| 2026-06-16 | `stages` → `section_wise_report_sqls`; `approval_requests` → `report_approvals`; `approval_decisions` → `report_approval_details`; `report_setups += is_setup_complete`; idempotency UNIQUEs on final_commissions/ev_disburse/pos_disbursement |
