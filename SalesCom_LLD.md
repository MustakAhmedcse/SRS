...
After migrating, run the **seed script** (`contracts/ddl/seed.sql`): `channels` (Distributor / RSO / Retailer), default `approval_flows` + `approval_flow_levels` (`approval_type` 1=Pre-Run / 2=Post-Run) + `approval_flow_level_users`, and the **permission catalog** (`user_rights.rights_code` codes — e.g. 25 `report_view` … 4.7). Roles (the bundles of these permission codes) and per-user assignment are **maintained in POS** and synced into `user_rights` (4.5), not seeded here. (Email/SMS templates live in code, not a DB column.)
...
**Identity:** `login_log.login_status` — 1 Success, 2 Failed. `user_rights.rights_code` — a per-action **permission** code (e.g. 25 `report_view`, 26 `report_create`, 31 `report_approval`, … full catalog in 4.7), **not** a role code. A user has one row per permission; roles are POS-defined bundles flattened into these rows.
...
| Rule | Enforcement |
|---|---|
| Access by assigned permission | JWT + per-action **permission** check against `user_rights` live (by `users.id`); roles are POS-defined bundles of permissions (4.7). |
